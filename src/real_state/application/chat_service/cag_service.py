...
class CAGService:
    ...
    def load_faqs(self, faq_queries: List[str]) -> int:
        ...
        Call warm_faqs() to generate responses
    ...
    def warm_faqs(self, verbose: bool = True) -> Dict[str, Any]:
        """
        Generate responses for FAQ questions and store in cache
        ...
        for i, query in enumerate(pending):
            ...
            rag_results = self.rag_service.generate(query)

            self.cache.update_faq_response(query, {
                'answer': rag_results['answer'],
                'evidence_urls': rag_results['evidence_urls']
            })
            warmed += 1
        ...
        if verbose:
            ...
            print(f" Warmed: {warmed}, Time: {total_time:.2f}s")
