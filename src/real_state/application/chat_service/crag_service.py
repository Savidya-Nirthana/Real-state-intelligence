...
- Better grounding and reduced hallucination

workflow:
    1. Initial retrieval (k=4)
    2. Calculate confidence score
    3. If low: Corrective retrieval (k=8)
...
from real_state.application.domain.utils import format_docs, calculate_confidence
...
class CRAGService:
    ...
    Works with QdrantStorage (search_chunks interface).
    ...
    Usage:
        service = CRAGService(retriever=qdrant_client, llm=llm)
    ...
    def __init__(
        ...
        Args:
            retriever: QdrantStorage instance (uses search_chunks API)
    ...
    def generate(
        ...
        # Step 1: Initial retrieval via QdrantStorage
        if verbose:
            print(f"1️⃣  Initial retrieval (k={self.initial_k})...")
        
        docs_initial = self.retriever.search_chunks(query=query, top_k=self.initial_k)
        ...
        if confidence_initial >= confidence_threshold:
            ...
            # Step 3: Corrective retrieval with expanded k
            if verbose:
                print(f"2️⃣  Corrective retrieval (k={self.expanded_k}, expanded)...")
            
            docs_corrected = self.retriever.search_chunks(query=query, top_k=self.expanded_k)
        ...
        # Format docs (plain dicts from QdrantStorage) and generate
        ...
        # Extract evidence URLs from dicts
        evidence_urls = list(set([doc.get('url', '') for doc in final_docs]))
    ...
    def analyze_confidence(self, query: str) -> Dict[str, Any]:
        ...
        # Expanded retrieval
        docs_expanded = self.retriever.search_chunks(query=query, top_k=self.expanded_k)
